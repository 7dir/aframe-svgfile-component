...
AFRAME.registerComponent('svgfile', {
  ...
  update: function(oldData){
    ...
    // oldData should be {ready:true} at this point if we are to continue further
...
>>>>>>> 865d1427ae7329a98cb40f95e094935b7ce3de79
    ...
    // data.width & data.height are our height and width in AFrame units
    // If only one is specified, the other is inferred from the aspect ratio.
...
>>>>>>> 865d1427ae7329a98cb40f95e094935b7ce3de79
    for (var ii=0; ii<allPaths.length; ii++){
        ...
        if (n.closed) {
            ...
            // Create polygon Geometry, using svgMesh3d to do the delauney triangulation
            var createGeometry = require('three-simplicial-complex')(THREE);

            __private_geometry = createGeometry(
              svgMesh3d(n.d, { // TODO: change from n.d to using n.path.getPathData() SVG2 interface
                delaunay: true,
                clean: true,
                exterior: false,
                randomization: 1,
...
<<<<<<< HEAD
            // Scale the geometry into AFrame units
            __private_geometry.scale(data.width/width, data.height/height, 1);


            // Add this geometry to the entity's Object3D
            el.setObject3D('svgmesh_'+ii, 
              new THREE.Mesh(
                __private_geometry, 
...
>>>>>>> 865d1427ae7329a98cb40f95e094935b7ce3de79
        ...
        } // Draw <path> as filled polygon?


        /* 
         * Draw <path> as meshline?
         * If the <path> has both a stroke and fill it will be rendered as a
         * polygon above and the border as a MeshLine here
         */
        if (n.strokeWidth>0)
...
<<<<<<< HEAD
              __private_geometry = svgPathDataToGeometry(n.path.getPathData(), {curveQuality: data.curveQuality});
              __private_geometry.scale(data.width/width, data.height/height, 1);
              // For some inexplicable reason, my THREE.Geometry from svgPathDataToGeometry
              // is *not* an instanceof THREE.Geometry inside MeshLine. I have *NO* idea why. So we convert to xyz array of verts
              var geometryAsArray = []; 
=======
...
>>>>>>> 865d1427ae7329a98cb40f95e094935b7ce3de79
              ...
              // Add this geometry to the entity's Object3D
              this.el.setObject3D("svgmesh_" + ii, 
                new THREE.Mesh(
                  line.geometry, 
...
/*
 This converts an SVG Path string (e.g. "M 0 0 L 20 20") into a THREE.Geometry with vertices defining
 the path. This vertex list geometry is suitable for being passed into THREE.MeshLine to be converted
into a triangle strip.
 */
function svgPathDataToGeometry(svgPathData, opts){
    var geometry = new THREE.Geometry();

    // Temporary variables used during the parsing process
    var v = new THREE.Vector3(0,0,0); 
    var c = null; // Temp variable, used to build THREE.CubicBezierCurve()s
    var x, tmp, p1, p2, c1, c2; // Temp vars
    var howManyCurves=0; // Many SVG commands can be repeated implicity, e.g. "L 5 5 20 15" means to draw two lines
    var values; // The numeric values after the SVG command. Array.
    var basisVector; // Used to track the basis point for subsequent relative coordinates
    var lastPenDown; // Used to support the path close ("Z") command.

    // Consider each SVG command
...
<<<<<<< HEAD
        ...
        case "l": // "l" may never be used, since SVGO may convert all "l" to "L"
          tmp.vertices.map(function(t){
            v = new THREE.Vector3(t.x+v.x, t.y+v.y, 0); // Convert to absolute coords. TODO: Possible logic bug here?!
            geometry.vertices.push(v.clone());
=======
        case "l":
          tmp = xyListToVertices(values);
...
>>>>>>> 865d1427ae7329a98cb40f95e094935b7ce3de79
        ...
        default:
          console.warn('Unrecognized SVG command:' + svgCommand.type);
      } // switch statement over various SVGCommand types
